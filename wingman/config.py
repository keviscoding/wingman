...
RAPID_FIRE_PROMPT = (
    ...
    "    – DATING-APP / MESSENGER UI BOILERPLATE: do NOT include any of "
    "      the following as 'messages', they are platform-generated UI "
    "      strings and are NOT messages either party typed:\n"
    "        • 'Start the chat with <name>'\n"
    "        • 'You liked <name>'s photo' / 'You liked their photo'\n"
    "        • 'You replied to their story' / 'You replied to <name>'s story'\n"
    "        • 'Your match expires in...' / 'It's a match!' / 'New match'\n"
    "        • 'You sent a like' / 'You sent a Super Like' / 'Boost'\n"
    "        • 'Read', 'Delivered', 'Sent', 'Active now', 'Online'\n"
    "        • Date/day separators ('Today', 'Yesterday', 'Wed, May 13')\n"
    "        • Any italicised/grey caption appearing between bubbles\n"
    "      These UI strings often look like messages but are added by\n"
    "      Hinge / Tinder / Bumble / Instagram / WhatsApp themselves.\n"
    "      They contaminate downstream chat-matching when included.\n"
...
MATCH_ADJUDICATOR_PROMPT = (
    ...
    "a different person.\n\n"
    "DEFAULT TO 'new' UNLESS THERE IS CONCRETE EVIDENCE OF CONTINUATION.\n\n"
    "Concrete evidence of CONTINUATION (what you need to merge):\n"
    "  • Substantive message text overlapping between the candidate's "
    "    recent tail and the screenshot (the user re-screenshot the "
    "    same conversation a second time, partially scrolled).\n"
    "  • A specific named entity, plan, joke, or shared detail that "
    "    appears in both (\"the climbing gym\", \"the dog Biscuit\", "
    "    \"Friday's plan\", etc.).\n"
    "  • Direct narrative continuity — the candidate's last messages "
    "    and the screenshot's first messages form a coherent thread.\n\n"
    "A screenshot is a NEW conversation when:\n"
    "  • Two DIFFERENT people happen to share a first name (extremely "
    "    common — e.g. two girls named Jess). Different banter style, "
    "    different topics, different rapport ALL count as evidence of "
    "    different people.\n"
    "  • The candidate has a long history (40+ messages) but the "
    "    screenshot's content does NOT pick up where that history "
    "    left off. Long shared vocabulary is NOT evidence of merge — "
    "    casual chat tone is universal. You need topical continuity.\n"
    "  • The opener matches between both but nothing after it does.\n"
    "  • Different bio details, different life context, different "
    "    phone number, different platform, different vibe.\n\n"
    "You will be shown 1-3 CANDIDATE CHATS (labeled A, B, C...) and "
    "the SCREENSHOT content. The candidate chats may have been trimmed "
    "to their most recent messages to keep this prompt focused — "
    "treat their tail as representative of the relationship's current "
    "state.\n\n"
    "Decisive principle: a shared first name + casual tone is NOT "
    "evidence of merge. Need shared topics / shared specifics / direct "
    "thread continuity to call it a merge.\n\n"
    "Return STRICT JSON:\n"
    "{\"verdict\": \"A\" | \"B\" | \"C\" | \"new\", "
    "\"confidence\": \"high\" | \"medium\" | \"low\", "
    "\"reason\": \"one short sentence citing the specific evidence\"}\n\n"
    "If your reason is anything weaker than \"shared specific topic / "
    "direct text overlap / clear thread continuity\", choose 'new'. "
    "Falsely merging two different people is far worse than splitting "
    "one person across two chats."
)
